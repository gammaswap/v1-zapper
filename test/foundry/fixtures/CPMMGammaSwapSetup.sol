...
contract CPMMGammaSwapSetup is UniswapSetup, TokensSetup {
    ...
    function setPoolParams(address _pool, uint16 origFee, uint8 extSwapFee, uint8 emaMultiplier, uint8 minUtilRate1, uint8 minUtilRate2,
        uint16 feeDivisor, uint8 liquidationFee, uint8 ltvThreshold, uint72 minBorrow) internal {
        vm.startPrank(address(factory));
        IGammaPool(_pool).setPoolParams(origFee, extSwapFee, emaMultiplier, minUtilRate1, minUtilRate2, feeDivisor, liquidationFee, ltvThreshold, minBorrow);// setting origination fees to zero
        vm.stopPrank();
    }
